...
class CifraFinal extends StatefulWidget {
  const CifraFinal({Key? key}) : super(key: key);

  static const String id = "cifrasFinal";

  @override
  State<CifraFinal> createState() => _CifraFinalState();
}

class _CifraFinalState extends State<CifraFinal> {
    ...
    for (int a = 0; a < textoCopia.length; a++) {
      for (int i = 0; i < 27; i++) {
        if (chaveAux![a] == matrix[i][0]) {
          print('Encontrou na coluna 0 linha $i: ${matrix[i][0]}');

          for (int j = 0; j < 27; j++) {
            if (textoCopia[a] == matrix[0][j]) {
              print('Encontrou na coluna $j linha 0: ${matrix[0][j]}');
              print('Resultado: linha $i coluna $j: ${matrix[i][j]}');
              traducao = traducao + matrix[i][j];
            }
          }
        }
      }
